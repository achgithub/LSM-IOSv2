...
/// Writes a Predictor game's CSV export to two temp files for the share
/// sheet — mirrors LMS's `GameExportFiles`.
enum PredictorExportFiles {
    ...
    static func write(for game: Game, data: LeagueData) throws -> [URL] {
        ...
        let metadataURL = try write(PredictorExportCSV.metadataCSV(for: game), named: "\(base) - metadata.csv")
        let predictionsURL = try write(PredictorExportCSV.predictionsCSV(for: game, data: data), named: "\(base) - predictions.csv")
