...
/// Loads team data, builds `KillerCardData`, renders via ImageRenderer @3x,
/// previews the result, and exposes the system share sheet. Mirrors
/// `PredictorShareView` exactly.
struct KillerShareView: View {
    ...
    private var title: String {
        ...
        case .playerKey:     return AppString("Player Key · Round \(round.roundNumber)")
        case .weeklyResults: return AppString("Results · Round \(round.roundNumber)")
        case .standings:     return AppString("Accuracy Table · Round \(round.roundNumber)")
    ...
    private func build() async {
        ...
        let data = KillerCardData.make(
            type: type, game: game, round: round,
            teamsById: teamsById, roundMatches: roundMatches
        )
        let renderer = ImageRenderer(
            content: KillerCardView(data: data).environment(\.locale, Bundle.appLocale)
        )
        renderer.scale = 3.0
