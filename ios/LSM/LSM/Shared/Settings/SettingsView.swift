import SwiftUI

/// Settings (spec §7.2) — a short list of icon rows, each pushing to its own
/// screen, matching Apple's own Settings app. Settings isn't a daily screen,
/// so scannability at a glance matters more than seeing every control at once.
struct SettingsView: View {
    @AppStorage(ManagerSettings.nameKey) private var managerName = ""
    @AppStorage(V2PreviewFlag.key) private var v2PreviewEnabled = true
    @Environment(Entitlements.self) private var entitlements
    @Environment(EnabledLeagues.self) private var enabled
    @Environment(LocalizationManager.self) private var localization

    var body: some View {
        NavigationStack {
            List {
                Section {
                    NavigationLink {
                        ProfileSettingsView()
                    } label: {
                        SettingsRow(
                            systemName: "person.crop.circle.fill",
                            title: "Profile", value: managerName.isEmpty ? nil : managerName
                        )
                    }
                    NavigationLink {
                        SubscriptionSettingsView()
                    } label: {
                        SettingsRow(systemName: "star.fill", title: "Subscription", value: entitlements.tier.label)
                    }
                    NavigationLink {
                        LeagueSettingsView()
                    } label: {
                        SettingsRow(systemName: "trophy.fill", title: "Leagues", value: "\(enabled.ids.count)/\(entitlements.leagueAllowance)")
                    }
                    NavigationLink {
                        RosterSettingsView()
                    } label: {
                        SettingsRow(systemName: "person.2.fill", title: "Roster")
                    }
                }

                Section {
                    NavigationLink {
                        LanguageSettingsView()
                    } label: {
                        SettingsRow(systemName: "globe", title: "Language", value: localization.language.displayName)
                    }
                    NavigationLink {
                        AboutView()
                    } label: {
                        SettingsRow(systemName: "info.circle.fill", title: "About")
                    }
                    NavigationLink {
                        ReportBugView()
                    } label: {
                        SettingsRow(systemName: "ladybug.fill", title: "Report a Bug")
                    }
                }

                Section {
                    Toggle(isOn: $v2PreviewEnabled) {
                        SettingsRow(systemName: "sparkles", title: "New design")
                    }
                } footer: {
                    Text("Switch on to use the redesigned app.")
                }
            }
            .listSectionSpacing(.compact)
            .appBackground()
            .navigationTitle("Settings")
        }
    }
}
